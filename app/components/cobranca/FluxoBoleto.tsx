...
interface FluxoBoletoProps {
  onVoltar: () => void;
  defaultValor?: string;
  defaultPagador?: string;
  defaultDescricao?: string;
  defaultVencimento?: string;
  onGerado?: () => void;
}

export function FluxoBoleto({ onVoltar, defaultValor, defaultPagador, defaultDescricao, defaultVencimento, onGerado }: FluxoBoletoProps) {
  const [step, setStep] = useState<"form" | "emitido">("form");
  const [pagador, setPagador] = useState(defaultPagador ?? "Armando Bagunça");
  const [valor, setValor] = useState(defaultValor ?? "149,00");
  const [vencimento, setVencimento] = useState(defaultVencimento ?? "09/09/2025");
  const [descricao, setDescricao] = useState(defaultDescricao ?? "Bol. ao pedido de venda nº 446");
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col items-center gap-4 flex-1">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted mt-1">Enviado para {pagador}</p>
          </div>

          <div className="w-full bg-white border border-border rounded-xl p-4 shadow-card">
            <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center font-bold text-white text-xs" style={{ background: "#2c6eff" }}>A</div>
              <div>
              ...
              <div className="ml-auto"><MeiooIcon size={16} variant="badge" /></div>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-border">
              <div className="flex gap-px h-8 items-end">
                {Array.from({ length: 60 }).map((_, i) => (
                  <div key={i} className="bg-dark flex-1" style={{ height: `${20 + Math.random() * 80}%` }} />
                ))}
              </div>
          ...
          <div className="flex items-center gap-2 text-[11px] text-muted bg-primary-subtle rounded-lg px-3 py-2 w-full">
            ...
            <span>Boleto emitido pela <strong className="text-dark">Meioo</strong> via conta Avante</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col gap-4 flex-1">
        <div>
          <label className="block text-[11px] font-semibold text-muted uppercase tracking-wide mb-1.5">Pagador</label>
          <input type="text" value={pagador} onChange={(e) => setPagador(e.target.value)}
        ...
        <div>
          <label className="block text-[11px] font-semibold text-muted uppercase tracking-wide mb-1.5">Descrição</label>
      ...
      <div className="flex gap-2 mt-6 pt-4 border-t border-border">
        ...
        <button onClick={handleEmitir} disabled={carregando}
          className="flex-1 h-9 rounded-lg bg-primary text-white text-xs font-semibold hover:bg-primary-hover transition-colors disabled:opacity-60">
          {carregando ? "Emitindo..." : "Emitir Boleto"}
        </button>
      </div>
    </div>
  );
