...
export function FluxoLink({ onVoltar, defaultValor, defaultDescricao, defaultVencimento, onGerado }: FluxoLinkProps) {
  const [step, setStep] = useState<"form" | "gerado">("form");
  const [valor, setValor] = useState(defaultValor ?? "320,00");
  const [descricao, setDescricao] = useState(defaultDescricao ?? "Pedido de venda nº 521");
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col items-center gap-4 flex-1">
          ...
          <div className="flex items-center gap-2 text-[11px] text-muted bg-primary-subtle rounded-lg px-3 py-2 w-full">
            ...
            <span>Link processado pela <strong className="text-dark">Meioo</strong> via conta Avante</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <div>
          <label className="block text-[11px] font-semibold text-muted uppercase tracking-wide mb-1.5">Formas de pagamento aceitas</label>
          <div className="flex gap-2">
            {["Pix", "Boleto"].map((meio) => (
              <label key={meio} className="flex items-center gap-1.5 text-xs text-dark cursor-pointer">
      ...
      <div className="flex gap-2 mt-6 pt-4 border-t border-border">
        ...
        <button onClick={handleGerar} disabled={carregando}
          ...
          {carregando ? "Gerando..." : <><ExternalLink size={13} />Gerar Link</>}
        </button>
      </div>
    </div>
  );
