...
export interface DadosPreenchidos {
  cliente: string;
  valor: number;
  historico: string;
  vencimento: string;
...
export function PainelCobranca({
  ...
  // helpers para pré-preencher campos
  const defaultValor = preenchido
    ? preenchido.valor.toLocaleString("pt-BR", { minimumFractionDigits: 2 })
    : undefined;

  return (
    <>
      {aberto && (
        <div className="fixed inset-0 bg-dark/30 z-40 transition-opacity" onClick={onFechar} />
      )}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {fluxo === "menu" && (
            <div className="flex flex-col gap-3">
              {preenchido && (
                <div className="bg-primary-subtle rounded-lg px-3 py-2.5 mb-1">
                  <p className="text-[11px] text-primary font-semibold">{preenchido.cliente}</p>
                  <p className="text-xs text-dark font-bold mt-0.5">
                    R$ {preenchido.valor.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                  </p>
                  <p className="text-[10px] text-muted mt-0.5">{preenchido.historico}</p>
                </div>
              )}
              <p className="text-xs text-muted mb-1">Escolha como quer receber esta cobrança:</p>
              {opcoes.map((op) => {
                const Icon = op.icon;
                return (
                  <button
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-semibold text-sm text-dark">{op.titulo}</span>
                        </div>
                        <p className="text-xs text-muted leading-relaxed">{op.descricao}</p>
              ...
              <div className="mt-2 p-3 rounded-xl bg-bg border border-border">
                <p className="text-[11px] text-muted leading-relaxed">
                  <strong className="text-dark">Incluir manualmente</strong> — Cadastre uma cobrança sem usar a conta digital Meioo.
                </p>
          ...
          {fluxo === "pix" && (
            <FluxoPix
              onVoltar={() => onFluxo("menu")}
              defaultValor={defaultValor}
              defaultDescricao={preenchido?.historico}
              defaultVencimento={preenchido?.vencimento}
              onGerado={onCobrancaGerada}
            />
          )}
          {fluxo === "boleto" && (
            <FluxoBoleto
              onVoltar={() => onFluxo("menu")}
              defaultValor={defaultValor}
              defaultPagador={preenchido?.cliente}
              defaultDescricao={preenchido?.historico}
              defaultVencimento={preenchido?.vencimento}
              onGerado={onCobrancaGerada}
            />
