...
export function FluxoPix({ onVoltar, defaultValor, defaultDescricao, defaultVencimento, onGerado }: FluxoPixProps) {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col items-center gap-4 flex-1">
          <div className="bg-white border-2 border-border rounded-xl p-4 shadow-card">
            ...
            >
              {Array.from({ length: 324 }).map((_, i) => (
                <div key={i} className={Math.random() > 0.5 || i < 63 || i > 261 ? "bg-dark" : "bg-white"} />
              ))}
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted mt-0.5">Vence em {vencimento}</p>
          </div>

          <div className="w-full bg-bg rounded-lg p-3">
            <p className="text-[10px] text-muted mb-1.5 font-medium uppercase tracking-wide">Código Pix Copia e Cola</p>
            <p className="text-[11px] text-dark break-all leading-relaxed font-mono">{pixCode.substring(0, 60)}...</p>
          </div>
          ...
          <div className="flex gap-2 w-full">
            ...
            >
              {copied ? <><CheckCircle2 size={13} />Copiado!</> : <><Copy size={13} />Copiar código</>}
            </button>
          ...
          <div className="flex items-center gap-2 text-[11px] text-muted bg-primary-subtle rounded-lg px-3 py-2 w-full">
            ...
            <span>Pix processado pela <strong className="text-dark">Meioo</strong> via conta Avante</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2c6eff" strokeWidth="2">
              ...
              <rect x="3" y="14" width="7" height="7" rx="1" /><path d="M14 14h.01M14 17h.01M17 14h.01M17 17h3M20 14v3" />
            </svg>
          </div>
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <p className="text-[11px] text-muted leading-relaxed">
          ...
          <span className="text-primary cursor-pointer hover:underline">tarifas da conta digital</span>.
        </p>
      </div>
      ...
      <div className="flex gap-2 mt-6 pt-4 border-t border-border">
        ...
        <button onClick={handleGerar} disabled={carregando}
          className="flex-1 h-9 rounded-lg bg-primary text-white text-xs font-semibold hover:bg-primary-hover transition-colors flex items-center justify-center gap-1.5 disabled:opacity-60">
          {carregando ? "Gerando..." : (
            <>
