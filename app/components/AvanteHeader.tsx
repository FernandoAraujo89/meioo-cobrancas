...
const AVANTE_PATHS = [
  ...
  "M30.8628 42.881H22.8387C21.9507 42.881 21.2305 42.1641 21.2305 41.2803C21.2305 40.3963 21.9507 39.6794 22.8387 39.6794H30.8628C31.7509 39.6794 32.471 40.3963 32.471 41.2803C32.471 42.881 31.7509 42.881 30.8628 42.881Z",
...
interface AvanteHeaderProps {
  onBusca?: (value: string) => void;
}

export function AvanteHeader({ onBusca }: AvanteHeaderProps) {
  const router = useRouter();
  const { startTour } = useMeiooTour();

  function handleRestartTour() {
    localStorage.removeItem("meioo_tour_v7");
    startTour();
  }
  ...
  return (
    <header className="h-14 bg-surface border-b border-border flex items-center px-0 shrink-0 z-10">
      ...
      <button
        onClick={() => router.push("/")}
        className="h-14 w-14 flex items-center justify-center shrink-0 hover:opacity-90 transition-opacity"
      ...
      <div className="flex-1 flex items-center justify-between px-3 sm:px-5 min-w-0">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button
            onClick={() => router.push("/")}
            className="font-semibold text-dark text-sm tracking-tight hidden sm:block hover:text-primary transition-colors"
          >
            Avante Web
          </button>
